...
import { Bell, CheckCircle, FileText, Award, MessageSquare, FolderOpen, Settings } from "lucide-react";

const TYPE_ICONS: Record<string, React.ElementType> = {
  application: FileText, scholarship: Award, message: MessageSquare,
  document: FolderOpen, system: Settings, commission: Award,
};
const TYPE_COLORS: Record<string, string> = {
  application: "bg-blue-100 text-blue-600", scholarship: "bg-amber-100 text-amber-600",
  message: "bg-purple-100 text-purple-600", document: "bg-orange-100 text-orange-600",
  system: "bg-slate-100 text-slate-600", commission: "bg-green-100 text-green-600",
};

const TABS = [
  { key: "all", label: "All" }, { key: "unread", label: "Unread" },
  { key: "application", label: "Applications" }, { key: "scholarship", label: "Scholarships" },
...
export default function NotificationsPage() {
  const allNotifs = usePortalStore(s => s.notifications.filter(n => n.recipientId === "s1"));
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-2xl space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <LiveBadge />
          {unreadCount > 0 && <span className="px-2 py-0.5 bg-blue-600 text-white text-xs font-bold rounded-full">{unreadCount}</span>}
        </div>
        {unreadCount > 0 && (
          <button onClick={() => markAllRead("s1")} className="flex items-center gap-1.5 text-xs text-blue-600 font-semibold hover:underline">
        ...
        {TABS.map(t => {
          ...
          return (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${tab === t.key ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
              {t.label}
              {cnt > 0 && <span className={`text-xs px-1.5 rounded-full ${tab === t.key ? "bg-blue-100 text-blue-600" : "bg-slate-200 text-slate-500"}`}>{cnt}</span>}
            </button>
          );
        })}
      </div>
        ...
        <div key={label}>
          ...
          <div className="space-y-2">
            {items.map(n => {
              ...
              return (
                <div key={n.id} onClick={() => markRead(n.id)} className={`flex items-start gap-3 p-4 rounded-2xl border cursor-pointer transition-all hover:shadow-sm ${n.read ? "bg-white border-slate-100" : "bg-blue-50 border-blue-100"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-1">{fmtTime(n.createdAt)}</p>
                  </div>
                </div>
              );
            })}
